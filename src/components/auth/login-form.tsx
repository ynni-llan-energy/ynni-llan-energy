...
export function LoginForm({ callbackError }: { callbackError?: string }) {
  ...
  const errorMessage = callbackError === "auth_callback_failed"
    ? "Methwyd mewngofnodi drwy Google / Google sign-in failed. Please try again."
    : callbackError === "oauth_failed"
    ? "Methwyd cysylltu â Google / Could not connect to Google. Please try again."
    : undefined;
  ...
  return (
    <div className="space-y-6">
      ...
      <form action={signInWithGoogle}>
        ...
        >
          <GoogleIcon />
          <span lang="cy">Mewngofnodi gyda Google</span>
          <span className="opacity-50 text-xs italic" lang="en">/ Sign in with Google</span>
        </button>
      </form>
      ...
      <form action={action} className="space-y-5" noValidate>
        ...
        <Button type="submit" disabled={pending} className="w-full" size="md">
          {pending ? (
            <span lang="cy">Yn mewngofnodi…</span>
          ) : (
            <>
              <span lang="cy">Mewngofnodi</span>
              <span className="opacity-60 italic text-xs" lang="en">/ Log in</span>
            </>
          )}
        </Button>
      </form>
