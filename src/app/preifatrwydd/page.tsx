...
export const metadata: Metadata = {
  title: "Preifatrwydd / Privacy — Ynni Cymunedol Llanfairfechan",
};

export default async function PrivacyPage() {
  const { isEnabled: isDraftMode } = await draftMode();
  const client = isDraftMode ? sanityDraftClient : undefined;
  const page = await getPage("page-preifatrwydd", client);

  const headingCy = page?.heading_cy ?? "Preifatrwydd";
  const headingEn = page?.heading_en ?? "Privacy";
